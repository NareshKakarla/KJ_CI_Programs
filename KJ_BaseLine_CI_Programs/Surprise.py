...
def Run_a_sample_path(V, initial_location, P):
    ...
    attempts = 0
    while P > C/(beta*temp_V) and temp_V >= C:
...
def plot_uav_path(coordinates, uav_positions):
    ...
    for i, (x, y) in enumerate(coordinates):
        ...
        if(i!=0):
            circle = plt.Circle((x, y), 8, fill=True, color='royalblue', alpha=0.2)  # Shaded, more visible circles
            plt.gca().add_artist(circle)
            # circle_edge = plt.Circle((x, y), 6, fill=False, color='black', alpha=0.4, linewidth=2)  # Circle edge
            # plt.gca().add_artist(circle_edge)
            plt.text(x, y, f'P{i}', fontsize=10, ha='right', va='bottom') 

    # Plot UAV path
    uav_x, uav_y = zip(*uav_positions)
    plt.plot(uav_x, uav_y, 'b-', linewidth=1.5, label='UAV Path')
    ...
    for i in range(len(uav_positions) - 1):
        ...
        plt.text(mid_x, mid_y, f'{distance:.2f}', fontsize=8, ha='center', va='bottom', bbox=dict(facecolor='white', edgecolor='none', alpha=0.7))

    plt.xlim(40, 100)
    plt.ylim(20, 80)
    plt.xlabel('X (km)', fontsize=10)
    plt.ylabel('Y (km)', fontsize=10)
    plt.title(f'UAV Flight Path (Initial V = {Initial_V}, P = {P_init})', fontsize=12)
    ...
    plt.legend(custom_lines, ['Nodes', 'UAV Path', 'UAV Positions', '6km Radius'], loc='upper left', fontsize=8)
    plt.savefig()
    plt.tight_layout()
    plt.show()
